Shader "Custom/RefractionShader" {
	Properties {
		...
		_RefractionAmount ("RefractionAmount", Float) = 0.1
	}
	SubShader {
		...
		void vert(inout appdata_full i, out Input o) {
			float4 worldPos = UnityObjectToClipPos(i.vertex);
			o.grabUV = ComputeGrabScreenPos(worldPos);
			o.refract = float4(i.normal,0) * _RefractionAmount;
		}

		void surf (Input IN, inout SurfaceOutputStandard o) {
			//float4 objPos = mul(unity_WorldToObject, float4(IN.worldPos, 1));
			//float4 uv_GrabTexture = ComputeGrabScreenPos(objPos);
